import React,{Component} from 'react';
import {Tabs , Tab , Card , CardActions,CardMenu ,Button,CardTitle, CardText, IconButton, Grid, Cell} from 'react-mdl'
class Projects extends Component {
    constructor(props){
        super(props);
        this.state = {activeTab : 0 };
    } 
    toggleCategories(){
        if (this.state.activeTab ===0){
            return (
                //django projects
                <div className = "projects-grid">
               <Card shadow = {5} style = {{minWidth: '450', margin :'auto'}}>
                   <CardTitle style = {{color:'#9A7D0A',height:'176px',background:
                   'url(https://static.djangoproject.com/img/logos/django-logo-negative.png) center/ cover'}}>Django Projects #1</CardTitle>
               <CardText>
                   asdfiijasfjasja;sjf;sjdfhasfkhasavh afhdsdhvaasd;uh afj;sdjvao
               </CardText>
               <CardActions border>
                   <Button colered>Github </Button>
                   <Button colered>Live Demo </Button>

               </CardActions>
               <CardMenu style = {{color : '#fff'}}>
                   <IconButton name = "share" />
               </CardMenu>
               </Card>
                {/* #2 project */}
               <Card shadow = {5} style = {{minWidth: '450', margin :'auto'}}>
               <CardTitle style = {{color:'#9A7D0A',height:'176px',background:
                   'url(https://static.djangoproject.com/img/logos/django-logo-negative.png) center/ cover'}}>Django Projects #2</CardTitle>
               <CardText>
                   asdfiijasfjasja;sjf;sjdfhasfkhasavh afhdsdhvaasd;uh afj;sdjvao
               </CardText>
               <CardActions border>
                   <Button colered>Github </Button>
                   <Button colered>Live Demo </Button>

               </CardActions>
               <CardMenu style = {{color : '#fff'}}>
                   <IconButton name = "share" />
               </CardMenu>
               </Card>
               <Card shadow = {5} style = {{minWidth: '450', margin :'auto'}}>
               <CardTitle style = {{color:'#9A7D0A',height:'176px',background:
                   'url(https://static.djangoproject.com/img/logos/django-logo-negative.png) center/ cover'}}>Django Projects #3</CardTitle>
               <CardText>
                   asdfiijasfjasja;sjf;sjdfhasfkhasavh afhdsdhvaasd;uh afj;sdjvao
               </CardText>
               <CardActions border>
                   <Button colered>Github </Button>
                   <Button colered>Live Demo </Button>

               </CardActions>
               <CardMenu style = {{color : '#fff'}}>
                   <IconButton name = "share" />
               </CardMenu>
               </Card>
               </div>
               
            
            )

        }else if(this.state.activeTab===1){
            // React projects 
            return(
                <Card shadow = {5} style = {{minWidth: '450', margin :'auto'}}>
                <CardTitle style = {{color:'#9A7D0A',height:'176px',background:
                'url(data:image/png;base64,) center/ cover'}}>React Projects #1</CardTitle>
            <CardText>
                asdfiijasfjasja;sjf;sjdfhasfkhasavh afhdsdhvaasd;uh afj;sdjvao
            </CardText>
            <CardActions border>
                <Button colered>Github </Button>
                <Button colered>Live Demo </Button>

            </CardActions>
            <CardMenu style = {{color : '#fff'}}>
                <IconButton name = "share" />
            </CardMenu>
            </Card>
            )
        }
    }
    render(){
    return (
   <div className = "category-tabs">
       <Tabs activeTab={this.state.activeTab} onChange={(TabId) => this.setState({activeTab:TabId})} ripple>
            <Tab>Django/Python</Tab>
            <Tab>React/Js</Tab>
       </Tabs>
       <Grid>
           <Cell col = {12} ><div className = "content">{this.toggleCategories()}</div></Cell>
       </Grid>
       
   </div>
    );
  }
}
  
  export default Projects;